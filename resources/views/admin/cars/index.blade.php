...
<div class="table-wrapper mb-4">
    ...
    $dataAttrs = [
        ...
        'data-brand'        => $c->brand ?? '',
        'data-model'        => $c->model,
        'data-photo'        => $c->photo ? asset('storage/'.$c->photo) : asset('assets/images/one_car/placeholder.png'),
...
@endphp
                ...
                >
                    ...
                    <td data-label="Фото">
                        @php
                            $display_photo = $c->catalog_photo ? asset('storage/'.$c->catalog_photo) : ( $c->photo ? asset('storage/'.$c->photo) : asset('assets/images/one_car/placeholder.png') );
                        @endphp
                        <img src="{{ $display_photo }}" alt="Фото" class="car-table-img" style="max-width:120px;">
                    </td>
                    ...
                    <td>
                        <button class="row-more-btn btn-light" data-bs-toggle="modal" data-bs-target="#viewRowModal" aria-label="Показать">
                            <i class="bi bi-three-dots-vertical"></i>
                        </button>
                        <input type="hidden" class="images-data" value='{{ json_encode($c->images->map(fn($img) => ['id' => $img->id, 'path' => asset('storage/'.$img->path), 'is_main' => $img->is_main])) }}'>
                    </td>
                </tr>
                @endforeach
            </tbody>
                ...
                <tbody class="text-center align-middle">
                    ...
                    <tr>
                        ...
                        <td data-label="Статус">
                            <form method="POST" action="{{ route('admin.cars.toggleBrand', $brand->id) }}" class="d-inline">
                                @csrf
                                @method('PUT')
                                <button type="submit" class="status-text" style="background: none; border: none; padding: 0; cursor: pointer; font-weight: 500; color: {{ $brand->is_active ? '#198754' : '#6c757d' }};">
            ...
            <form method="POST" action="{{ route('admin.cars.addBrand') }}">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="brand_name_input" name="brand_name" maxlength="100" placeholder="Введите новую марку">
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <form id="addCarForm" method="POST" action="{{ route('admin.cars.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                <select id="add_brand" name="brand" class="form-select @error('brand') is-invalid @enderror" required>
                                    <option value="">Выберите марку...</option>
                                    @foreach ($existingCarBrands as $existingBrand)
                                        <option value="{{ $existingBrand }}" {{ old('brand') == $existingBrand ? 'selected' : '' }}>
                                            {{ $existingBrand }}
                                        </option>
                                    @endforeach
                                </select>
                                <button class="btn btn-outline-primary" type="button" data-bs-toggle="modal" data-bs-target="#addBrandModal" title="Добавить новую марку">
                            ...
                            @enderror
                            <div class="form-text">Выберите марку из списка. Если нужной марки нет, нажмите <strong>+</strong> чтобы добавить</div>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input id="add_model" name="model" type="text"
                                ...
                                maxlength="30" placeholder="LX 500d" value="{{ old('model') }}">
                            @error('model')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="add_drive" name="drive" class="form-select @error('drive') is-invalid @enderror">
                        ...
                        <div class="col-md-6">
                            ...
                            <input id="add_engine" name="engine" type="number" step="0.1" min="0.1" max="20.0"
                        ...
                        <div class="col-md-4">
                            ...
                            <select id="add_fuel" name="fuel" class="form-select @error('fuel') is-invalid @enderror">
                                <option value="">Выберите...</option>
                        ...
                        <div class="col-md-4">
                            ...
                            <input id="add_mileage" name="mileage" type="number" min="0" max="10000000"
                                ...
                                placeholder="140000" value="{{ old('mileage') }}">
                            @error('mileage')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select id="add_condition" name="condition" class="form-select @error('condition') is-invalid @enderror">
                                <option value="">Выберите...</option>
                        ...
                        <div class="col-md-6">
                            ...
                            <input id="add_owners" name="owners" type="number" min="0" max="100"
                                ...
                                placeholder="1" value="{{ old('owners') }}">
                            @error('owners')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input id="add_transmissions" name="transmissions" type="number" min="1" max="20"
                                class="form-control @error('transmissions') is-invalid @enderror"
                        ...
                        <div class="col-md-6">
                            ...
                            <input id="add_trunk" name="trunk" type="number" min="1" max="10000"
                                class="form-control @error('trunk') is-invalid @enderror"
                                placeholder="701" value="{{ old('trunk') }}">
                            @error('trunk')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="add_gearbox" name="gearbox" class="form-select @error('gearbox') is-invalid @enderror">
                                <option value="">Выберите...</option>
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="add_body" name="body" class="form-select @error('body') is-invalid @enderror">
                                <option value="">Выберите...</option>
                        ...
                        <!-- Catalog photo -->
                        <div class="col-12">
                            <label for="add_catalog_photo" class="form-label required-field">Фото для карточки каталога (главное)</label>
                        ...
                        <!-- Доп. фото -->
                        <div class="col-12">
                            <label for="add_images" class="form-label required-field">Дополнительные фото для галереи</label>
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn-primary w-100 btn"><i class="bi bi-save me-1"></i> Сохранить</button>
                </div>
            </form>
        </div>
    </div>
            ...
            <form id="editCarForm" method="POST" action="" enctype="multipart/form-data">
                ...
                <div class="modal-body p-3">
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            <label for="edit_catalog_photo" class="form-label">Новое фото для каталога (оставьте пустым, если не меняете)</label>
                        ...
                        <div class="col-md-6">
                            ...
                            <input id="edit_brand" name="brand" type="text"
                                ...
                                maxlength="20" placeholder="Lexus" value="{{ old('brand') }}">
                            @error('brand')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="edit_drive" name="drive" class="form-select @error('drive') is-invalid @enderror">
                                <option value="">Выберите...</option>
                                <option value="Полный" {{ old('drive') == 'Полный' ? 'selected' : '' }}>Полный</option>
                                <option value="Передний" {{ old('drive') == 'Передний' ? 'selected' : '' }}>Передний</option>
                                <option value="Задний" {{ old('drive') == 'Задний' ? 'selected' : '' }}>Задний</option>
                            </select>
                            @error('drive')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6">
                            ...
                            <input id="edit_engine" name="engine" type="number" step="0.1" min="0.1" max="20.0"
                                class="form-control @error('engine') is-invalid @enderror"
                                placeholder="5.0" value="{{ old('engine') }}">
                            @error('engine')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select id="edit_fuel" name="fuel" class="form-select @error('fuel') is-invalid @enderror">
                                <option value="">Выберите...</option>
                                ...
                                <option value="Электро" {{ old('fuel') == 'Электро' ? 'selected' : '' }}>Электро</option>
                            </select>
                            @error('fuel')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-4">
                            ...
                            <select id="edit_condition" name="condition" class="form-select @error('condition') is-invalid @enderror">
                                <option value="">Выберите...</option>
                                <option value="Новая" {{ old('condition') == 'Новая' ? 'selected' : '' }}>Новая</option>
                                <option value="Не битая" {{ old('condition') == 'Не битая' ? 'selected' : '' }}>Не битая</option>
                                <option value="Битая" {{ old('condition') == 'Битая' ? 'selected' : '' }}>Битая</option>
                                <option value="Аварийная" {{ old('condition') == 'Аварийная' ? 'selected' : '' }}>Аварийная</option>
                            </select>
                            @error('condition')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6">
                            ...
                            <input id="edit_transmissions" name="transmissions" type="number" min="1" max="20"
                                class="form-control @error('transmissions') is-invalid @enderror"
                                placeholder="8" value="{{ old('transmissions') }}">
                            @error('transmissions')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <!-- Багажник -->
                        <div class="col-md-6">
                            <label for="edit_trunk" class="form-label required-field">Вместимость багажника (L)</label>
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="edit_gearbox" name="gearbox" class="form-select @error('gearbox') is-invalid @enderror">
                                <option value="">Выберите...</option>
                                <option value="Автомат" {{ old('gearbox') == 'Автомат' ? 'selected' : '' }}>Автомат</option>
                                <option value="Механика" {{ old('gearbox') == 'Механика' ? 'selected' : '' }}>Механика</option>
                                <option value="Робот" {{ old('gearbox') == 'Робот' ? 'selected' : '' }}>Робот</option>
                                <option value="Вариатор" {{ old('gearbox') == 'Вариатор' ? 'selected' : '' }}>Вариатор</option>
                            </select>
                            @error('gearbox')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="edit_body" name="body" class="form-select @error('body') is-invalid @enderror">
                                <option value="">Выберите...</option>
                                <option value="Кроссовер" {{ old('body') == 'Кроссовер' ? 'selected' : '' }}>Кроссовер</option>
                                <option value="Седан" {{ old('body') == 'Седан' ? 'selected' : '' }}>Седан</option>
                                <option value="Хэтчбек" {{ old('body') == 'Хэтчбек' ? 'selected' : '' }}>Хэтчбек</option>
                                <option value="Универсал" {{ old('body') == 'Универсал' ? 'selected' : '' }}>Универсал</option>
                                <option value="Купе" {{ old('body') == 'Купе' ? 'selected' : '' }}>Купе</option>
                                <option value="Кабриолет" {{ old('body') == 'Кабриолет' ? 'selected' : '' }}>Кабриолет</option>
                                <option value="Внедорожник" {{ old('body') == 'Внедорожник' ? 'selected' : '' }}>Внедорожник</option>
                                <option value="Минивэн" {{ old('body') == 'Минивэн' ? 'selected' : '' }}>Минивэн</option>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                <input id="edit_price" name="price" type="number" min="0" max="1000000000"
                                    class="form-control @error('price') is-invalid @enderror"
                                    placeholder="12500000" value="{{ old('price') }}">
                                <span class="input-group-text">₽</span>
                                @error('price')
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn-primary w-100 btn"><i class="bi bi-save me-1"></i> Сохранить изменения</button>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="modal-content">
            <div class="modal-header d-flex align-items-center justify-content-between">
                <div>
                    ...
                    <div class="small text-muted">ID: <strong id="v_id">—</strong> · Пробег: <strong id="v_mileage">—</strong></div>
                </div>
                ...
                <div class="row">
                    <div class="col-12 mb-4">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="small text-muted">ID: <strong id="v_id_2">—</strong> · Пробег: <strong id="v_mileage_2">—</strong></div>
                        </div>
                    </div>
                    <div class="col-12 mb-4">
                    ...
                    <div class="col-12">
                        ...
                        <div class="data-value" id="v_description" style="line-height: 1.6; white-space: pre-line;">—</div>
                    </div>
                </div>
            </div>
            <div class="modal-footer justify-content-between">
                <div>
                    <form method="POST" action="" onsubmit="return confirm('Удалить автомобиль? Эта операция необратима.');">
                        @csrf
                        @method('DELETE')
...
function initModalHandlers() {
    ...
    // Кнопки "Редактировать" в модалке просмотра
    ...
    // Заполнение данных при клике на строку
    ...
    // Обработка поля "Новая" → пробег и владельцы
...
function initBrandManagement() {
    ...
    document.querySelectorAll('.edit-brand-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            document.getElementById('editBrandForm').action = '{{ route("admin.cars.updateBrand", "") }}/' + this.dataset.brandId;
        });
    });

    // Синхронизация select и input при добавлении марки
...
function fillViewModal(data, images) {
    ...
    badges.innerHTML = '';
    if (data.drive) badges.insertAdjacentHTML('beforeend', `<span class="badge bg-primary me-1">${data.drive}</span>`);
    if (data.engine) badges.insertAdjacentHTML('beforeend', `<span class="badge bg-secondary me-1">${data.engine}</span>`);
    if (data.fuel) badges.insertAdjacentHTML('beforeend', `<span class="badge bg-success me-1">${data.fuel}</span>`);
    if (data.gearbox) badges.insertAdjacentHTML('beforeend', `<span class="badge bg-info text-dark me-1">${data.gearbox}</span>`);
    ...
    document.querySelector('#viewRowModal .modal-footer form').action = '{{ route("admin.cars.destroy", "") }}/' + data.id;
}
...
window.openEditModalWithData = function(data, images) {
    ...
    // Пробег (самое важное исправление)
    ...
    // Применяем логику "Новая" после заполнения
