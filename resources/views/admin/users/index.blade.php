    ...
    <div class="container-xxl section">
        ...
        <div class="card mb-4 border-0 shadow-sm" style="border-radius: 12px;">
            <div class="card-body d-flex align-items-center justify-content-between flex-wrap gap-3">
                <div>
                    <h5 class="card-admin mb-1" style="font-size: 1.1rem; font-weight: 600;">
                        <i class="bi bi-shield-check me-2 text-primary"></i>Проверка email при регистрации
                    </h5>
                    <p class="text-muted mb-0" style="font-size: 0.9rem;">
                        При отключении регистрация возможна без подтверждения почты (полезно при проблемах с VPN/сетью)
                    </p>
                </div>
                <div class="form-check form-switch" style="transform: scale(1.2);">
                    <input class="form-check-input" type="checkbox" id="emailVerificationToggle"
                        {{ $emailVerificationEnabled ? 'checked' : '' }}>
                    <label class="form-check-label visually-hidden" for="emailVerificationToggle">
                        Включить проверку email
                    </label>
                </div>
            </div>
            <div id="verificationStatusAlert" class="card-footer bg-transparent border-0 pt-0 pb-3 px-4"
                style="display: none;">
                <span class="badge" id="verificationStatusBadge"></span>
            </div>
        </div>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="password-input-wrapper">
                                <input type="password" class="form-control form-control-with-icon" id="add_password"
                ...
                <form id="changePasswordForm" action="" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="password-input-wrapper">
                                <input type="password" class="form-control form-control-with-icon" id="cp_new_password"
                                    name="new_password" required minlength="6">
                                <button type="button" class="password-toggle"
                                    onclick="togglePassword('cp_new_password')">
                                    <i class="bi bi-eye"></i>
                                </button>
                            </div>
                            <div class="form-text">Минимум 6 символов</div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="password-input-wrapper">
                                <input type="password" class="form-control form-control-with-icon"
                                    id="cp_confirm_password" name="new_password_confirmation" required minlength="6">
                                <button type="button" class="password-toggle"
                                    onclick="togglePassword('cp_confirm_password')">
                                    <i class="bi bi-eye"></i>
                                </button>
                            </div>
...
@push('scripts')
                ...
                .then(async response => {
                    ...
                    if (contentType && contentType.includes('application/json')) {
                        ...
                        if (response.status >= 400) {
                            throw {
                                type: 'server',
                                message: data.errors?.database?.[0] || data.errors?.self?.[0] || data.message ||
                                    'Ошибка'
                            };
                        }
                        return data;
    ...
    `;
        ...
        // Переключение проверки email
        document.getElementById('emailVerificationToggle')?.addEventListener('change', function() {
            const toggle = this;
            const csrfToken = document.querySelector('meta[name="csrf-token"]')?.content;
            ...
            fetch('{{ route('admin.users.toggle-email-verification') }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken,
                        'Accept': 'application/json',
                ...
                .then(async response => {
                    ...
                    if (response.ok && data.success) {
                        ...
                        const alertBox = document.getElementById('verificationStatusAlert');
                        const badge = document.getElementById('verificationStatusBadge');
                        if (alertBox && badge) {
                            alertBox.style.display = 'block';
                            badge.textContent = data.enabled ? 'Проверка включена' :
                                'Проверка отключена — регистрация без email';
                            badge.className = 'badge ' + (data.enabled ? 'bg-success' :
